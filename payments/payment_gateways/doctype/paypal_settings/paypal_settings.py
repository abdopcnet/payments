	...
	def validate_paypal_credentails(self):
		...
		try:
			...
			if res.get("ACK") and res["ACK"][0] == "Failure":
				error_msg = res.get("L_LONGMESSAGE0", [_("Invalid payment gateway credentials")])[0]
				frappe.throw(error_msg)

		except frappe.exceptions.ValidationError:
			raise
		except Exception as e:
			frappe.log_error(f"PayPal API Error: {str(e)}\nResponse: {res if 'res' in locals() else 'No response'}")
			frappe.throw(_("Unable to validate PayPal credentials. Check error log for details."))
