...
class PayPalSettings(Document):
    ...
    def setup_sandbox_env(self, token):
        data = json.loads(frappe.db.get_value(
            "Integration Request", token, "data"))
    ...
    def validate_paypal_credentails(self):
        ...
        try:
            ...
            if res.get("ACK") and res["ACK"][0] == "Failure":
                error_msg = res.get("L_LONGMESSAGE0", [
                                    _("Invalid payment gateway credentials")])[0]
            ...
            frappe.log_error(
                f"[paypal_settings.py] validate_credentials: {str(e)}")
            frappe.throw(
                _("Unable to validate PayPal credentials. Check error log for details."))
    ...
    try:
        ...
        update_integration_request_status(
            ...
            {"payerid": response.get("PAYERID")[
                0], "payer_email": response.get("EMAIL")[0]},
            "Authorized",
            doc=doc,
        )
        ...
        frappe.log_error(
            f"[paypal_settings.py] get_express_checkout_details: {str(e)}")
...
def confirm_payment(token):
    try:
        ...
        if response.get("ACK")[0] == "Success":
            ...
            if data.get("reference_doctype") and data.get("reference_docname"):
                custom_redirect_to = frappe.get_doc(
                    data.get("reference_doctype"), data.get(
                        "reference_docname")
        ...
        frappe.log_error(f"[paypal_settings.py] confirm_payment: {str(e)}")
...
def create_recurring_profile(token, payerid):
    try:
        ...
        starts_at = starts_at.replace(tzinfo=ZoneInfo(
            get_system_timezone())).astimezone(ZoneInfo("UTC"))
        ...
        frappe.log_error(
            f"[paypal_settings.py] create_recurring_profile: {str(e)}")
...
def ipn_handler():
    try:
        ...
        frappe.log_error(f"[paypal_settings.py] ipn_handler: {str(e)}")
...
def handle_subscription_notification(doctype, docname):
    call_hook_method("handle_subscription_notification",
