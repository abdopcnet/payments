...
class MpesaSettings(Document):
    ...
    def handle_api_response(self, global_id, request_dict, response):
        ...
        if error:
            frappe.throw(_(response.errorMessage),
...
def generate_stk_push(**kwargs):
    ...
    try:
        ...
        frappe.log_error(f"[mpesa_settings.py] get_payment_request: {str(e)}")
        frappe.throw(
    ...
    if transaction_response["ResultCode"] == 0:
        if integration_request.reference_doctype and integration_request.reference_docname:
            try:
                ...
                frappe.log_error(
                    f"[mpesa_settings.py] verify_transaction: {str(e)}")
...
def get_account_balance(request_payload):
    ...
    try:
        ...
        frappe.log_error(f"[mpesa_settings.py] get_account_balance: {str(e)}")
        frappe.throw(
...
def process_balance_info(**kwargs):
    ...
    if account_balance_response["ResultCode"] == 0:
        try:
            ...
            frappe.log_error(
                f"[mpesa_settings.py] process_account_balance: {str(e)}")
    else:
        request.handle_failure(account_balance_response)
...
def create_mode_of_payment(gateway, payment_type="General"):
    ...
    payment_gateway_account = frappe.db.get_value(
        "Payment Gateway Account", {
            "payment_gateway": gateway}, ["payment_account"]
    )
